...
- A `denyRead` pattern covers its own tree: the literal directory the pattern starts from (what is written before its first wildcard) and everything under it. That is the pattern's directory, not the project's: for `src/**/.env` the tree is `src`. A symlinked directory is descended only when it leads to somewhere inside that tree, judged by where the link resolves and not by how it is written. What this gives up: a file that the pattern matches only by a name that passes through a link out of the tree is not masked. With `src/shared` a link to `../shared`, `src/**/.env` does not mask `shared/.env`, which stays readable under both names; a pattern that starts at the file's directory or above it (`shared/**/.env`, `**/.env`), or an entry naming the file itself, does. Such a file can be inside the tree, when a link out there leads back in: with `src/vendor` a link to `../vendor` and `vendor/secrets` a link to `../src/data`, `src/**/secrets/*.pem` matches `src/data/key.pem` as `src/vendor/secrets/key.pem` alone and does not mask it, under either name; a pattern that matches it by a name inside the tree (`src/**/*.pem`) masks it under every name. A link whose own name matches the pattern still masks what it leads to, wherever that is: a matched link to a file masks the file, and a matched link to a directory hides the whole directory; when that directory is outside the tree it is not listed.
