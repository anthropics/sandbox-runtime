...
Prefer a non-root caller where there is the choice. Under the seccomp isolation layer a root caller's command still holds a full capability set inside the helper's nested user namespace, which is identity-mapped to the caller's uid 0; what holds the filesystem policy there is the helper's namespaces filter, which refuses that command the calls that change a mount tree (next paragraphs). The nested namespace's copies of the mounts are locked as well, which refuses the unmount of any one deny, but not the whole tree being moved aside with `pivot_root` and dropped with a lazy unmount, so the locks alone do not hold it. A non-root caller's command holds no capabilities at all.

**The command stays in the namespaces made for it.** Every write deny on Linux is a read-only bind mount, and a bind protects a path only in the mount namespace it was made in. Creating a user namespace takes no capability and gives its creator a full capability set over a private copy of the mount tree, from which the binds can be removed; so a sandboxed command is not allowed to create one. Two independent things enforce that when the seccomp isolation layer is in use: a second seccomp filter beside the Unix-socket one (`unshare` and `clone` are refused `EPERM` when they ask for a user namespace; `clone3`, whose flags a filter cannot read, is answered `ENOSYS`, which sends libc and the language runtimes to `clone`), and `user.max_user_namespaces` set to zero in the helper's own user namespace. The same filter refuses, with `EPERM`, `setns`, `mount`, `umount2`, `pivot_root` and the newer mount calls (`open_tree`, `move_mount`, `fsopen`, `fsconfig`, `fsmount`, `fspick`, `mount_setattr`, `open_tree_attr`), whatever their arguments. Where the layer is not in use (`allowAllUnixSockets`, or an architecture with no helper) bubblewrap is given `--disable-userns`, which refuses the creation of a user namespace (with `ENOSPC`) and nothing else: the other calls are answered by the kernel, and `--cap-drop ALL`, which such a command always runs under, leaves them nothing to change a mount tree or join a namespace with. `--disable-userns` needs bubblewrap 0.8.0 or later and a bubblewrap that is not installed setuid; with an older one `checkDependencies()` warns, under the code `bwrap_lacks_disable_userns`, that nothing enforces it.

What stops working under the sandbox as a result is whatever sandboxes itself with user namespaces: Chromium's and Electron's namespace sandbox, rootless podman and buildah, a nested bubblewrap or `srt`, `unshare -r` in a build script. What such a program reports depends on who refused it. Under the seccomp isolation layer the error is `EPERM` (`unshare: unshare failed: Operation not permitted`), which a nested bubblewrap explains as a kernel setting (`No permissions to create a new namespace, likely because the kernel does not allow non-privileged user namespaces`, with a `sysctl` to set): that advice does not apply here. Without the layer, under `--disable-userns`, it is `ENOSPC` (`No space left on device`; bubblewrap: `nesting depth or /proc/sys/user/max_*_namespaces exceeded`), while `/proc/sys/user/max_user_namespaces` read from inside still shows a large number, the limit being on the namespace above. Either way the cure is `allowNestedUserNamespaces: true`, not a sysctl: it lifts all of the above (the filter, the limit and `--disable-userns`) for such a command, at the price stated under that option. For a command without privileges the one other visible difference is that `open_tree` is refused even without `OPEN_TREE_CLONE`, a form that needs no privilege and that almost nothing uses. A root caller's command additionally loses `mount`, `umount` and the rest of that family for purposes of its own (a tmpfs, a bind, a `/proc` for a chroot build), which it could use inside the helper's namespace before, and with them `unshare -m`, which sets the propagation of the new tree with a `mount` call (`--propagation unchanged` avoids it), and `setns` (`nsenter`, `ip netns exec`); the same option gives them back. Threads and child processes are unaffected: `clone` without the user-namespace flag passes, and libc and the common language runtimes fall back to it when `clone3` answers `ENOSYS`. What has no fallback is a direct `clone3` call and the few interfaces built on one, such as starting a child straight into another cgroup.

A root caller's command keeps its capabilities inside the helper's namespace (previous paragraph), so it could raise the limit again; for it the filter is what counts, and the mount calls are in the filter for that reason. `enableWeakerNestedSandbox` does not lift any of this, but under it `/proc/sys` is usually read-only, so the limit cannot be written and the filter stands alone there. There the command can also still see the helper's outer process in `/proc`; the helper makes itself non-dumpable before it forks (always, `allowNestedUserNamespaces` or not), and takes the namespaces filter for that process too, so that a root caller's command cannot reach into it. Without the seccomp isolation layer as well, nothing enforces it, and `checkDependencies()` says so under the code `no_userns_limit_in_weaker_nested_sandbox` and reports `features.usernsLimit` as `false`. The wrap and that check take the decision from one function, so what is reported is what is done.

An embedder that compiles the helper into its own binary (`seccomp.argv0`) must rebuild it from this release's `vendor/seccomp-src` to get any of this: the JavaScript half only passes the option on. `checkDependencies()` reports what it found as data, `features.usernsLimit` (`true`, `false`, or `'unknown'` when the helper could not be asked: one that only exists inside the sandbox, or no bubblewrap that could be made to ask it, which needs to be allowed a network namespace for that even where your commands run without one), and warns under the code `helper_lacks_userns_limit` when a helper answers that it has no limit. The library asks a helper that is a file of its own inside bubblewrap, in namespaces of its own with nothing writable, no network and no capabilities, and never runs it any other way: the file may be one a sandboxed command was allowed to write. A file put in its place can there still read what your user can read, and connect to a Unix socket that has a path. Any helper can be asked directly: run with `SRT_HELPER_FEATURES=1` in its environment it prints one feature a line (`userns-limit`) and exits 0 without doing anything else, and one built before the question existed fails to run the name it was given instead.
