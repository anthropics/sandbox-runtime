...
- A pointer is read the way git reads one — the whole file, `\n` and `\r` stripped from its end, the path ending at the first NUL — and one larger than the 1 MiB git accepts for a `.git` file is not followed, because git refuses it too. A pointer and a `commondir` are both denied under the path as written and, where a `..` in one follows a symlink, under the directory the kernel actually opens as well, since `link/../x` does not land where folding the path on paper says it does.
- A directory a pointer or a `commondir` names that this cannot read is denied whole instead — git, running as the same user, cannot read through it either — and so is a directory under `.git/modules` the submodule walk cannot see through. A whole-directory deny leaves everything beneath it read-only inside the sandbox, a submodule's `objects`, `refs` and `index` included, so git writes inside such a tree stop working. Two things produce one under `.git/modules`: a directory the walk cannot list and an entry it cannot stat, and for both of them what is denied is the deepest ancestor the walk can reach, which may be `.git/modules` itself.
- The `.git/modules` walk has no depth bound: a submodule's name is its path and submodules nest, so how deep a tree goes is a question about the repository, and a submodule a hundred levels down has its hooks and config denied like any other. Each directory is visited once, keyed by where it really is, so a symlinked entry pointing back into the tree ends that branch rather than looping. What bounds the walk is time — the same budget the `ripgrep` scan gets on Linux, since a tree that takes longer than that to walk is one the command about to run could have made — and running out of it refuses the command (`deny_scan_failed`) rather than sandboxing on the submodules the walk did reach.
- **Past what bubblewrap takes arguments for, the submodule denies are collapsed (Linux).** A profile takes three of bubblewrap's 9000 words per mount, and a submodule git directory costs five of them: four denies and an ancestor pin. A repository with thousands of submodules therefore asks for more than the profile has room for, and the ceiling is not this library's to lift — so the denies are degraded instead, fail-closed and only as far as they have to be. The working directory's own repository is served first, then the nested repositories the scan found, in sorted order:
  1. precise denies (`hooks`, `config`, `commondir`, `config.worktree`) while they fit;
  2. the submodule git directories that do not fit are denied WHOLE, one read-only bind each — git writes inside those submodules fail read-only, their `objects`, `refs` and `index` with them, and nothing outside them changes. Which ones is settled by sorted order, the last first, so two wraps of one tree collapse the same submodules and a command is sandboxed the same way each time;
  3. where even one bind each does not fit, the enclosing `.git/modules` is denied whole and every submodule under it goes with it.
     One warning per wrap says how many were collapsed and at which level. `too_many_arguments` is still there for a profile that does not fit after all of this, which now takes something other than submodules to cause.
...
**Refusals a wrapped command can plant (Linux).** Two of the wrap-time refusals below follow from a file a sandboxed command is allowed to write, and the command that would put it right is refused as well, because every wrap in that working directory hits the same file. Both name the offending path, or the line the scan printed, in the message, and **both are lifted only by a command run outside the sandbox**:
...
A directory the command makes unreadable does **not** refuse anything: it is denied whole, so the next command can neither read it nor open it up again.
