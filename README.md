...
- A `denyRead` pattern covers its own tree: the literal directory the pattern starts from (what is written before its first wildcard) and everything under it. That is the pattern's directory, not the project's: for `src/**/.env` the tree is `src`. A directory outside that tree is never listed: a symlinked directory is descended only when it leads to somewhere inside the tree, judged by where the link resolves and not by how it is written, whether or not the link's own name matches and whatever is allowed beneath what it leads to. A link out of the tree can only add to what a pattern covers: a file whose real path is inside the tree and matches is found by walking the tree itself, whatever links exist, so no link a sandboxed command creates takes a mask away. What this gives up is what such a link used to add: a file that the pattern matches only by a name that passes through a link out of the tree is not masked. With `src/shared` a link to `../shared`, `src/**/.env` does not mask `shared/.env`, which stays readable under both names; a pattern that starts at the file's directory or above it (`shared/**/.env`, `**/.env`), or an entry naming the file itself, does. Such a file can be inside the tree, when a link out there leads back in: with `src/vendor` a link to `../vendor` and `vendor/secrets` a link to `../src/data`, `src/**/secrets/*.pem` matches `src/data/key.pem` as `src/vendor/secrets/key.pem` alone and does not mask it, under either name; a pattern that matches it by a name inside the tree (`src/**/*.pem`) masks it under every name. It can be a file written later, too: a command that may write in the tree can move a directory that holds nothing masked out of it and leave a link in its place, and what is written under that name afterwards lies outside the tree and is not masked; `unfollowedDenyLinks` names the link. A link whose own name matches the pattern still masks what it leads to, wherever that is: a matched link to a file masks the file, and a matched link to a directory hides the whole directory. When that directory is outside the tree it is not listed, so an `allowRead` or `allowWrite` path beneath it is bound back as written, as beneath a directory listed in `denyRead` literally, and what the pattern matches under that path through the link's name is not masked again: with `src/build` a link to `../out/build`, `src/**/build/**` and an `allowRead` of `out/build/pub`, everything in `pub` is readable and the rest of `out/build` is hidden, under either name. What a link inside such a directory leads to is not masked either: with `out/build/data` a link to `../../keep`, `keep` stays readable.
- Inside the tree, a directory is listed once for each way the pattern can carry on beneath it, however many links lead to it, so the cost of the expansion follows the size of the tree and the length of the pattern, not the number or length of the names its links offer; what is found through a link is reported, and denied, where it really is. A link that leads back up the tree (to the directory holding it or above, or to the pattern's starting directory or above) is not descended. A `**` written against other text (`**.pem`, `a**/x`) and a bracket expression that can match `/` span directories as they do on macOS, through symlinked directories too. Only a pattern that does not read as written is matched against real paths alone, with every directory under its starting directory listed: one with a wildcard inside a bracket expression (`[a*]`), or with a second `[` that nothing closes. A link that itself matches it still denies what it leads to. Every `denyRead` mount goes where the path really is (bubblewrap 0.12 and later refuse to mount on a symlink), so an entry reached through a symlink is denied under every name that leads to it, and a link back up the tree denies everything it reaches, as a literal deny of the link would. A link that resolves to nothing is skipped. `allowRead` globs are not expanded through symlinks: they match the link itself.
- The expansion of a configuration's `denyRead` patterns has a budget, shared by all of them: 2,000,000 directory entries looked at and 10 seconds. The time a tree takes depends on the machine and on what is cached, so the first command after start-up in a very large tree can be refused where a second is not. When either runs out the wrap is refused with a `LinuxSandboxProfileError` whose `.code` is `deny_glob_too_large`, and no command is returned: a deny list cut short would leave readable what the pattern was written to hide. `getFsReadConfig()` throws the same error. The message names the pattern, the directory being listed and what had been spent; narrow the pattern, or remove or move what it walks into. The same are fields of the error's `.cause`, for a caller that words its own message: `pattern` (the one being expanded when the budget ran out, which need not be the large one), `directory`, `exhausted` (`'entries'` or `'time'`), `entries` and `elapsedMs` (what all the patterns had spent between them), `maxEntries` and `timeoutMs`. The time is checked before each directory is listed and at each entry in it, so what is not cut short is a single step: a listing that blocks (a dead network mount inside the tree), the resolution of a link that leads to one (where a link leads is looked up even when it is then not descended), or one name that is slow to match (a pattern with several wildcards in one path component, against a long name). The budget is the walk's: what is done with the matches afterwards is not counted. `getFsReadConfig()` lists the links the patterns left unfollowed for leading out of their tree in `unfollowedDenyLinks`, each with its `pattern`, `link` and `target`, for a caller to tell its user what a pattern does not reach. The list is of those links only: a link that leads back up the tree, and any link under a pattern that cannot be followed one path component at a time, is not listed through either and is not in it. With debug logging on (`SRT_DEBUG`) each expansion logs one line: the pattern, the milliseconds it took, its matches and mounts, the directories listed, the entries looked at, and how many links out of the tree it left unfollowed, with the first of them.
