...
- `network.allowLocalBinding` - Allow binding to local ports (boolean, default: false). On macOS the outbound side of this allowance is `(remote ip "localhost:*")`, which matches `127.0.0.1` and `::1` but not the IPv4-mapped form `::ffff:127.0.0.1` that a dual-stack socket (`AF_INET6` with `IPV6_V6ONLY` off — the JVM's default, and what gRPC's C core does, see #494) produces when it connects to an IPv4 literal; Seatbelt's host token is limited to `localhost` or `*`, so the mapped form cannot be admitted without also admitting non-loopback destinations. Such a connect fails with `EPERM`: use an `AF_INET` socket, or for the JVM rely on the `-Djava.net.preferIPv4Stack=true` that srt already injects (see "JVM tools").
