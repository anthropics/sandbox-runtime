...
- Mach Service Registration (macOS): The `network.allowMachRegister` option lets the sandboxed process publish Mach service names via `bootstrap_check_in` (`(allow mach-register ...)`), which Chromium-based browsers need in order to register their `MachPortRendezvousServer` and Crashpad ports. This is a wider grant than the `allowMachLookup` counterpart: looking up only lets the sandboxed process reach a service that already exists, whereas registering publishes a name into the login session's bootstrap namespace, where processes **outside** the sandbox resolve it. A sandboxed command holding a broad pattern can therefore claim a name whose real server has not started yet and receive the IPC that server's clients intended for it. A name already held by a running service cannot be taken over, so the exposure is to not-yet-registered names rather than live ones. Scope the patterns to the prefixes the command genuinely registers (e.g. `"com.google.chrome.for.testing.*"`) instead of `"*"`. It only works together with `allowMachLookup` for the same names: registering publishes the port, and the browser’s child processes still need to look it up — without the lookup the browser hangs instead of failing, so `srt` warns when a registered name has no lookup covering it. The name depends on the build: Chrome for Testing uses `com.google.chrome.for.testing.*`, Playwright’s bundled Chromium uses `org.chromium.Chromium.*`.
