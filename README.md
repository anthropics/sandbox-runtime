...
- `network.deniedDomainReasons` - Optional map from a `deniedDomains` entry (matched by exact string) to a model-facing reason that appears in the `<sandbox_violations>` line when that entry denies a connection — say what is blocked and the sanctioned alternative (e.g. `{"github.com:22": "SSH pushes to GitHub are blocked; use an https:// remote"}`). Entries without a reason report a generic one. The reason is also delivered in-band, on the paths whose client reads it rather than inferring a broken proxy: a plain-HTTP request through the proxy gets it as the body of the `403` (`X-Proxy-Error: blocked-by-allowlist`), and an SSH client tunneled through a no-auth SOCKS ProxyCommand (e.g. BSD `nc -X 5`) receives a pre-key-exchange SSH disconnect whose description is the reason, which OpenSSH prints verbatim — keep such reasons under ~400 ASCII characters, imperative first, since OpenSSH truncates and escapes non-ASCII. A denied `CONNECT` — i.e. any `https://` URL — carries the same `403` body, but no common client shows it: curl prints `curl: (56) CONNECT tunnel failed, response 403` (even under `-v`), git prints `CONNECT tunnel failed, response 403`, and Python's urllib raises `Tunnel connection failed: 403 Forbidden`. For those the reason reaches the model through the `<sandbox_violations>` line instead, via `SandboxManager.annotateStderrWithSandboxFailures` — the `srt` CLI does not print that block. Because a sandboxed process can read the text on the other paths, do not put anything in it that the process should not see.
- `network.allowlistDenyReason` - Optional reason reported when a host is denied because no `allowedDomains` entry matched it, in place of the generic `host is not on the allow list` in the `<sandbox_violations>` line. Setting it also replaces the proxy's `403` body, which is otherwise the unchanged `Connection blocked by network allowlist` — so this is where to tell an agent that the block is policy and not a transport fault, e.g. `"blocked by sandbox policy; do not retry with --no-proxy, another DNS server or a VPN"`. It reaches a client on the same paths a `deniedDomainReasons` reason does, with the same `CONNECT` caveat above: an `https://` client still prints only the `403` status. A plain SOCKS5 denial carries no text either: the protocol has only a reply code.
