...
**Write denies on paths that do not exist yet (Linux):** bubblewrap can only deny a path by mounting over it, so for a `denyWrite` path that is absent under a writable directory it first creates a mount point there: an empty, read-only file (or an empty directory for a missing intermediate component) that is visible on the host for as long as a sandbox is alive and is removed once no running sandbox relies on it. Host tools therefore see such a path as existing while a sandboxed command runs, which matters for paths whose existence is their meaning (a lockfile such as `.git/config.lock` makes `git config` report "could not lock config file"). Each wrap records the mount points it relies on in a manifest, a small JSON file in a per-user directory, and passes it to bubblewrap as `--lock-file`: the sandbox's init process holds a kernel lock on the manifest for exactly the sandbox's lifetime, however the sandbox ends, and `/proc/locks` shows that lock to any process in the same PID namespace. So any srt process there can tell whether a mount point is still in use, whoever made it. `cleanupAfterCommand()` removes the ones no running sandbox relies on, this process's and other processes' alike, and calling it early, twice or not at all takes nothing away from a sandbox that is running. A process that dies without an exit event (`SIGKILL`, OOM) cannot remove its own mount points; the next cleanup in any process does, once the manifest is more than half a second old. An empty regular file with no write bits found at a `denyWrite` path under a writable directory is taken to be such a leftover: it is covered with `/dev/null` like an absent path and removed after the command. A leftover empty directory cannot be told from anyone else's and is left alone, unless the manifest of a sandbox that may still be running names it: then a wrap that denies that same directory names it too, so that it stays for as long as either sandbox runs.

- **The manifest directory** is `$XDG_RUNTIME_DIR/srt-mount-points` when `XDG_RUNTIME_DIR` is set, else `<os.tmpdir()>/srt-mount-points-<uid>`. It has to be a real directory (a symlink at the name is refused), owned by the user, mode `0700` (it is made so), and writable; a candidate under `/dev`, `/proc` or `/sys` is passed over (the sandbox mounts a fresh `/dev` and `/proc` over whatever was bound there, and bubblewrap could then not open the manifest). When neither name will do (both are read-only from where the process runs, as inside another sandbox), a process that has mount points to record makes a directory of its own under the temp dir, and removes it again at `reset()` or on exit once it is empty: its own sandboxes are covered as before, but no other process sees its manifests or collects what it leaves behind. With no directory at all, mount points are not recorded and this process leaves them on the host. Every sandbox that restricts writes has both of those names bound read-only, as the wrapping process sees them and whichever of them it uses itself (it makes the one that is not there), and its own directory if that is neither, whether or not its wrap has mount points. So a sandboxed command can neither delete a manifest nor forge one, nor make the directory first, for a process that was started with `XDG_RUNTIME_DIR` and for one that was started without. The `directory.lock` file in it keeps srt processes out of each other's way while they collect; nothing depends on it, and a pass never waits more than two seconds for it.
- **Commands in parallel.** Call `cleanupAfterCommand()` once for each wrapped command, when that command is over. A call cannot tell which command it is for, so with no argument nothing this process made is released until it has been called once for every wrap it handed out: a command that is wrapped and not yet started still needs its manifest (bubblewrap refuses to start without it, "Unable to open lock file") and its `--args` profile. One long-running command therefore holds back the cleanup of the ones that finish beside it. To avoid that, pass the `commandId` the wrap was given, `cleanupAfterCommand({ commandId })`: that one command's mount points go at once, whatever else is running. A call too many takes nothing from a running sandbox, but it is counted for some other wrap of this process, and a command that has not started by the time the count is back at zero then refuses to start. What other srt processes have finished with is collected by every call. `reset()` and the exit handler release everything this process made; a sandbox of this process that is still running then keeps its mount points, through its lock, and they go at the next cleanup in any process that uses the same manifest directory.
- **Known limits.**
  - _Another PID namespace._ `/proc/locks` lists a lock only when its holder has a pid in the PID namespace of the `/proc` being read, and `/proc/<pid>` is local to it too. A manifest therefore records the PID namespace it was written in, and a process in any other one (a container that shares `/tmp` or `XDG_RUNTIME_DIR` with the host, one srt inside another's sandbox) takes it for live: it never removes what that manifest names, and an empty file, or a directory, found at a path it names counts as a mount point for that process's own wraps. Only a process in the namespace that wrote a manifest collects it, so what a namespace that has since gone left behind (empty files, empty directories, the manifests) stays until it is removed by hand.
  - _A sandbox this release did not start, or a directory it does not know of._ The read-only binds of the manifest directories are in the sandboxes this release starts, for the two names as the wrapping process sees them. A sandbox started by an older release, or by another tool, that can write the temp dir while a manifest directory is under it (no `XDG_RUNTIME_DIR`) can delete and forge manifests, and so can a sandbox of this release when another process of the same user keeps its manifests somewhere the wrapping process does not look (a different `TMPDIR`, a different `XDG_RUNTIME_DIR`) and the sandbox may write there. A cleanup on the host believes what it finds in its directory. It still removes nothing but an empty, unwritable, singly linked file of the user's, or an empty directory of the user's.
  - _Between looking and removing._ The directory lock does not exclude: a holder can be held up past the minute after which its lock is broken, and a wrap that cannot have the lock within two seconds records its manifest without it. A cleanup pass therefore reads `/proc/locks` and lists the directory again immediately before it removes anything, and then again whenever its listing is a quarter of a millisecond old, and what it read from `/proc/locks` two milliseconds old or 64 removals ago. It removes a manifest last, after everything it names, and keeps one whose mount point it could not remove. A pass that is held up for longer than a sandbox takes to start (a few milliseconds) between such a look and a removal can still take away the mount point of a sandbox that started on the same path in that time.
  - _Mount points recorded where another process does not look, or not at all._ Two processes see each other's manifests only when they settle on the same manifest directory. One that cannot write the first choice (it runs inside another sandbox, where that directory is read-only) records in the next one, in a private one, or nowhere, and then nothing the other process reads says a sandbox relies on those mount points. A wrap in that other process that denies the same path takes the file for a leftover, covers it, and removes it after its own command, whether or not the first sandbox is still running. The ones that could not be recorded at all are never removed by the process that made them either.
  - _Exit under a running sandbox._ A process that exits, or calls `reset()`, while a sandbox it started is still running leaves that sandbox's mount points and manifest in place, as it must. They go at that process's own next cleanup if it lives on after `reset()`, and otherwise at the next cleanup by another srt process that uses the same manifest directory; with a private directory, no other process does.
...
- The string must be run while the process that produced it is alive, and before the runtime cleans up after that command (`cleanupAfterCommand()`), which is when the profile and the wrap's mount point manifest are released: with several commands in flight, when the last of them has been cleaned up after (the manifest alone goes earlier when the command's own `commandId` is passed). A string run after that refuses to start rather than running without its mount points.
...
- Every one of these wrap-time refusals is a `LinuxSandboxProfileError`, exported from the package root, with a `LinuxSandboxProfileErrorCode` on `.code` to tell the cases apart; branch on `.code` rather than on the message. They say the configuration expands to a profile this host cannot run, except `command_too_long` and `nul_in_path`, which also fire on what the embedding program passed in. A wrap that threw has already let go of what it held and is not counted among the commands to clean up after: `cleanupAfterCommand()` need not be called for it, and with other commands in flight should not be, since the call would be taken for one of theirs.
