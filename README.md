...
- `git worktree remove`, `git worktree move`, `git worktree repair`, `git submodule deinit`, for the same reason, and `git worktree prune` (and the prune `git gc` runs) for a worktree the repository still keeps, whose `.git/worktrees/<id>/` entries are denied;
