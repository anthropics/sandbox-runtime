...
Two limits. A configuration that allows writes everywhere and carves out denies (`allowWrite: ['/']` with `denyWrite` entries) gets none of this filtering: every location is inside an allowed write path, so choosing a copy would mean judging file permissions, which the library does not do. And the fallback that looks for `apply-seccomp` in a global npm install still runs `npm root -g` through `PATH`; it is reached only when the binary is in none of the package's own locations and neither `seccomp.applyPath` nor `seccomp.argv0` is set, which an npm install of this package never is.
