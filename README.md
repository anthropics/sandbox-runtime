...
- `network.allowLocalPorts` - macOS only: loopback TCP ports the sandboxed process may bind, accept on and connect to (array of 1-65535, e.g. `[9222]` for a headless browser's devtools port), without opening every port the way `allowLocalBinding` does. Independent of `allowLocalBinding` (both may be set). No-op on Linux (loopback inside the sandbox's network namespace is already private) and on Windows.
  - The per-port rule matches `127.0.0.1` / `::1` but not the IPv4-mapped form `::ffff:127.0.0.1`, so a dual-stack runtime (Java etc.) must bind AF_INET for it to match; JVMs get `-Djava.net.preferIPv4Stack=true` injected, as they do for the proxy ports.
