...
**Pinned directories (Linux):** Each existing directory between a protected path (a write-denied path, a read-denied file or directory, a masked credential file) and the outermost allowed write root that contains it cannot be renamed or removed from inside the sandbox: `mv` or `rmdir` of such a directory (for example a nested repository's parent) fails with `EBUSY` ("Device or resource busy"), and `rm -rf` of a nested repository leaves the pinned directories and the protected files behind (as with `.git/hooks`). Reads, writes, creation and renames inside or across a pinned directory are unaffected. Nothing is pinned when `/` itself is an allowed write root: that root's own bind is applied over every pin, so renaming a directory under it behaves as if it had none.
