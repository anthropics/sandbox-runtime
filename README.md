...
**The library's own files.** Installed as a dependency of a project, this package lives in `<project>/node_modules/`, inside what `allowWrite` usually names, and its `dist/`, its bundled seccomp helper and the packages it loads are what the host runs the next time `srt` is started there. So what this copy is loaded from is write-denied to every wrapped command whenever it lies inside an allowed write path: the package's own directory and that of every package it depends on at run time, found the way the module loader finds them; every place the loader looks for one of those before the place it is found (`node_modules/@anthropic-ai/node_modules/`, which does not exist and could otherwise be created to be loaded instead); and, on macOS, the package's launcher in `node_modules/.bin/`. Nothing else under `node_modules` is touched. What it costs: a wrapped `npm ci`, which removes `node_modules` whole, or an `npm install` that has to touch one of those packages, fails in a project that has this package as a dependency; install it globally, or run it with `npx` from outside the project, to keep those working. Nothing is denied for a checkout of this repository, a global install outside the write paths, or a copy compiled into an application.

Two limits. On Linux the launcher `node_modules/.bin/srt` is a link whose own name a mount cannot hold, so a wrapped command can still re-point it: start the library as `node node_modules/@anthropic-ai/sandbox-runtime/dist/cli.js`, or from an install outside the write paths, where that matters. And what is protected is what the library itself is loaded from; a program of your own that loads it is found from wherever that program is.
