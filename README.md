...
- the [mandatory deny paths](#mandatory-deny-paths-auto-protected-files) — `.git/hooks`, `.git/config`, the dangerous dotfiles and directories — resolved against the working directory, as the same write-DENY ACE. No configuration lifts these: a path that is both granted and denied gets both ACEs, and the deny is written ahead of the allow. Only the ones that are already there are covered, and only the working directory's own; the mandatory-deny section has the reasons
...
- **Mandatory deny paths are narrower than on Linux and macOS.** The list is applied, resolved against the working directory, but only for paths that already exist and only for the working directory's own — a dangerous file the sandboxed command creates for the first time, and a nested repository's `.git/hooks`, are not blocked. See [Mandatory Deny Paths](#mandatory-deny-paths-auto-protected-files) for why, and for the `filesystem.denyWrite` entry that covers a name before it exists.
...
- **Windows**: Writes additive `(OI)(CI)` explicit ACEs for the `srt-sandbox` SID onto the configured paths (ALLOW on `allowRead`/`allowWrite`, DENY on `denyRead`/`denyWrite`, plus the [mandatory deny paths](#mandatory-deny-paths-auto-protected-files) that are already there in the working directory), then removes them at `reset()`
...
- **Only paths that are already there are covered.** A Windows deny is an NTFS ACE, and an ACE needs an object to sit on; the placeholder chain `srt-win acl stamp` creates for a missing `filesystem.denyWrite` target is permanent by design (`acl restore` strips the ACE and keeps the object, so a caller who wrote into one cannot lose data), which is not something to do to a name nobody asked for. So a dangerous file the sandboxed command creates for the first time is not blocked. List it in `filesystem.denyWrite` to get the placeholder behaviour for it.
- **Only the working directory's own paths are covered.** Linux finds nested matches with a depth-limited `ripgrep` scan and macOS gets them from its glob patterns; a Windows deny is one ACE per path with no pattern form, so a nested repository's `.git/hooks` and a dangerous file in a subdirectory are not blocked.
