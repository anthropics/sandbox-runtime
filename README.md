...
- `network.deniedDomainReasons` - Optional map from a `deniedDomains` entry (matched by exact string) to a model-facing reason that appears in the `<sandbox_violations>` line when that entry denies a connection — say what is blocked and the sanctioned alternative (e.g. `{"github.com:22": "SSH pushes to GitHub are blocked; use an https:// remote"}`). Entries without a reason report a generic one. The reason is also delivered in-band, so the sandboxed client itself reads it rather than inferring a broken proxy: an HTTP or CONNECT request through the proxy gets it as the body of the `403` (`X-Proxy-Error: blocked-by-allowlist`), and an SSH client tunneled through a no-auth SOCKS ProxyCommand (e.g. BSD `nc -X 5`) receives a pre-key-exchange SSH disconnect whose description is the reason, which OpenSSH prints verbatim — keep such reasons under ~400 ASCII characters, imperative first, since OpenSSH truncates and escapes non-ASCII.
- `network.allowlistDenyReason` - Optional reason reported when a host is denied because no `allowedDomains` entry matched it, in place of the generic `host is not on the allow list`. It reaches the same two places a `deniedDomainReasons` entry does — the `<sandbox_violations>` line and the proxy's `403` body — so it is where to tell an agent that the block is policy and not a transport fault, e.g. `"blocked by sandbox policy; do not retry with --no-proxy, another DNS server or a VPN"`. A plain SOCKS5 denial still carries no text: the protocol has only a reply code.
