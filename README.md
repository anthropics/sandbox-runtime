...
- `bwrapPath`, `socatPath` - Linux: absolute path of the `bwrap` / `socat` to run, used as given instead of being looked up on `PATH` (see "Where these programs are taken from" under Platform-Specific Dependencies)
- `ripgrep` - `{ command, args?, argv0? }`: the ripgrep to run for the Linux scan. A bare `command` (the default, `rg`) is looked up as described there; one with a directory part is run as given
- `seccomp` - Linux: `{ applyPath?, argv0? }`: the `apply-seccomp` helper to use instead of the bundled one
...
**Where these programs are taken from.** `bubblewrap`, `socat` and `ripgrep` are run by the library itself, on the host, outside the sandbox. They are looked up on `PATH` inside the process (no `which` is run), and a copy inside a path the sandboxed command may write (`filesystem.allowWrite` and the default write paths) is passed over, as is one reached through a symbolic link kept in such a path, or through a relative `PATH` entry: such a file is whatever an earlier sandboxed command left there. This matters when `PATH` leads with a directory of the project, as under `npx` and `npm run` (`<project>/node_modules/.bin`) with the project in `allowWrite`. When no copy is left, `initialize()` and `checkDependencies()` report an error naming the copies passed over, and a wrap throws `LinuxSandboxProfileError` with code `host_helper_unavailable`. Install the program outside the write paths, or name it with `bwrapPath`, `socatPath` or `ripgrep.command` (a path); a program named that way is run as given, and `checkDependencies()` warns when the sandboxed command may write it. The shell the command runs in is still found on the whole `PATH`: it runs inside the sandbox. On macOS `env`, `sandbox-exec` and `log` are run from `/usr/bin`.

Two limits. A configuration that allows writes everywhere and carves out denies (`allowWrite: ['/']` with `denyWrite` entries) gets none of this filtering: every location is inside an allowed write path, so choosing a copy would mean judging file permissions, which the library does not do. And where `apply-seccomp`, or the JVM agent's jar, is in none of the package's own locations (and no `seccomp.applyPath`, `seccomp.argv0` or `javaAgentJarPath` names it), the library asks `npm root -g` for a global install. From `initialize()` and the asynchronous dependency check, `npm` and the `node` it starts are looked up on a `PATH` with the same places left out; a synchronous dependency check made before either has run still uses the `PATH` as it is. An npm install of this package carries both files and asks nothing.
...
- Filesystem Permission Escalation: Overly broad filesystem write permissions can enable privilege escalation attacks. Allowing writes to directories containing executables in `$PATH`, system configuration directories, or user shell configuration files (`.bashrc`, `.zshrc`) can lead to code execution in different security contexts when other users or system processes access these files. The library's own host-side helpers are not taken from such a directory (see "Where these programs are taken from"), but anything else run from it is.
