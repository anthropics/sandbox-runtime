...
- For every linked worktree a repository keeps, `commondir`, `gitdir` and `config.worktree` inside `.git/worktrees/<id>/` are denied as well, whether or not that worktree's checkout is anywhere the command can write: rewriting `commondir` there redirects the hooks and config the host's git uses in that other worktree, and what an existing `commondir` names is denied with it. They are listed for the working directory's own repository and each of its submodules' git directories (`.git/modules/<name>/worktrees/<id>/`, where `git worktree add` inside a submodule puts one) and, on Linux, for each nested repository the scan finds. A session whose working directory is itself a linked worktree reaches its own git directory through its `.git` file; its sibling worktrees' are not listed from that end.
- A directory a pointer or a `commondir` names is a git directory by what it holds (`HEAD`, `objects` or `commondir`) or by where it is: `.git/worktrees/<id>`, and anything under a `.git/modules`, is one with every marker moved aside. Where it is is judged on the path the kernel lands on, so a symlinked `.git` cannot lend that shape to an ordinary directory. Renaming `HEAD` inside a linked worktree's git directory therefore drops no deny.
...
- A directory a pointer or a `commondir` names that this cannot read is denied whole instead — git, running as the same user, cannot read through it either — and so is a directory under `.git/modules` the submodule walk cannot see through. A whole-directory deny leaves everything beneath it read-only inside the sandbox, a submodule's `objects`, `refs` and `index` included, so git writes inside such a tree stop working. Two things produce one under `.git/modules`: a directory the walk cannot list, and an entry whose target is there and cannot be inspected. What is denied is the deepest directory that can be reached towards it — towards the TARGET for an entry that is a symlink, not the directory holding it, which would be every submodule beside it — and that can be `.git/modules` itself where the walk cannot get past it. An entry that is a symlink loop reaches no directory at all, so nothing is denied for it: there is nothing behind it to protect, and a bind cannot be put at the link's own location either, since mounting there means resolving it. A pointer or a `commondir` whose VALUE reaches nothing is not denied whole anywhere either — neither the directory it would fold to nor the deepest one still reachable towards it, which for a value like `gitdir: a/gd` with `a` a loop in the checkout is the checkout itself. git opens nothing through such a value, and a `.git` file is something a sandboxed command is deliberately allowed to create, so failing closed there would let one command leave every command after it unable to write the project. What is denied is the pointer file and the links the walk did go through, with one `SRT_DEBUG` warning naming the file.
- **An entry that is itself a symlink is two things, and both are denied.** `hooks`, `config`, `commondir`, `config.worktree`, `modules` and an entry of a `.git/modules` can each be a link: what a write through it reaches is one thing, the link itself — which a command can unlink and leave its own `hooks/` in place of, for the host's git to run before the next command's scan sees anything — is another. Which of the two one deny covers is the backend's: a Linux bind lands on what the path resolves to, while Seatbelt compares its filter against the path the kernel resolved the operation to — which for an unlink or a rename is the link's own path, since the last component is not followed, and for a write through the link is the target. So the target is denied on its own, and on Linux the directory holding the link is denied WHOLE: the git directory for one of its own entries, the directory holding a `.git/modules` entry for one of those. Everything under a whole deny is read-only, so in a repository that has such a link `git status`, `log` and `diff` go on working while `git add`, `git commit` and anything else writing the index or an object fail read-only until the link is gone; on macOS nothing is denied whole and git keeps working, the entry's own deny being what holds the link there. A link whose target is **not there yet** is no exception: where it lands is denied as a git directory that does not exist, so the command cannot create it and leave hooks for the host's git. An entry this cannot classify at all — an `lstat` that fails for anything but absence — is taken for a link, both denies and all. A link that is a loop reaches nothing, so only the whole deny is left to it.
- **`modules` is held as a link, and is no deny path of its own.** A git directory keeps its submodules' git directories under `modules`, each with precise denies of its own, so denying that directory whole would take every one of them read-only. But it is an entry like the others in the one respect that matters: remove the link, put a directory of your own where it was, and every submodule git directory the repository keeps is one nothing denied - which is a hook the host's git runs. So it is held the way a chain hop is, by its own name where a filter matches the name a rename uses, and by the directory holding it - that git directory, denied whole - where a deny resolves. This applies to the top-level `.git/modules` as much as to a submodule's own. **What it costs, and that a command can arrange it:** an ordinary repository has no `.git/modules` at all, and an absent one is deliberately not denied (a mount there would stop `git submodule add`), so one `ln -s` inside the sandbox makes `.git` a git directory with a symlinked entry - and from the next wrap on `git add`, `git commit` and `git checkout -b` fail read-only in that repository, with nothing inside the sandbox able to remove the link. `git status`, `log` and `diff` keep working, and removing the link from outside the sandbox puts it back. That is the same family as the other costs a command can plant, and it is the price of holding the link at all.
- **A chain is held hop by hop, not just at its two ends.** Neither end of a chain need point straight at the other. An entry can go through a link on the way: `hooks -> ../hooks-link` with `hooks-link -> .githooks`, or `hooks -> ../shared/hooks` with `shared` a symlinked directory. So can the VALUE of a `gitdir:` pointer or a `commondir`, which is a path git walks as the kernel does: `gitdir: hop/../evil`, or a git directory reached through a symlinked component. The file's or the entry's deny holds that end and the landing's deny holds the other; a link BETWEEN them is held by neither, and a command that retargets it — or renames it aside and puts its own directory there — moves where the chain resolves without touching either end, for the host's git to follow. So every hop is held as the entry or the pointer file is: its own path is denied, which is what holds it where a filter matches the name a rename or an unlink uses, and on Linux the directory HOLDING it is denied whole, which is what holds it where a deny resolves. On macOS a chain needs one thing more. A deny path whose own spelling goes through a link — everything under the git directory `gitdir: links/shared/gd` names, and everything under a submodule reached through a symlinked `.git/modules` entry — is compared against a resolved path it can never equal, so it holds the link's own name and nothing beneath it. Every path the git denies were read off disk is therefore named twice there, as it was written and at the landing the kernel walks it to, absent tails included; the two are the same string, and one entry, wherever no link is involved. A HOP is the exception and keeps its own name alone: what a hop leads to is a whole directory nothing asked to deny, so denying it would take everything beside the git directory with it, and the link's own name is the whole handle on one — the mirror image of Linux dropping a hop link from its binds. What lies past the hop is the git directory, whose own deny paths carry both spellings. A hop inside the git directory, or inside the directory holding a `.git/modules` entry, needs no directory of its own: the whole deny that entry already brings covers it. One that no allowed write path contains is read-only anyway. The chain is walked as the kernel walks it, so what a hop cannot be read past leaves the git directory denied whole, as an entry that cannot be classified does, and the hops before it are held all the same. **A chain that reaches NOTHING is held by its hops alone.** A loop, or a chain longer than the kernel follows, has no landing: git gives up on it exactly as this does, so the deny the landing would get stands on nothing, and every link the walk visited before it gave up is the whole of what a command can move — put a directory in one of their places and the chain reaches it, and the host's git reads the hooks or config inside (tested against real git). Each is therefore denied like any other hop, named once however many times the loop met it. A loop that stays inside the git directory needs nothing more: the whole-directory deny that a symlinked entry already brings covers its links. One that runs out through the work tree is held by the directories holding those links — and where THAT cannot be done, because such a directory is the working directory or a write root, Linux **refuses the command** (`deny_unresolvable_git_entry`), since the warning that stands in for a bind elsewhere leans on both ends of the chain being denied and here there is no far end. That refusal is what such a repository got before any of this existed, when the deny path simply could not be resolved. A link ABOVE the entry or the pointer file is not a hop of the chain at all: a checkout reached through a symlink is where the repository lives, and the walk starts where the kernel already is.
- **Known limit: a hop the working directory or a write root holds.** `.git/hooks -> ../hooks-link` with `hooks-link` in the repository root is the common shape of the above — and so is a pointer whose value goes through a link in the checkout itself, where the directory holding the hop is the working directory by construction — and there is nothing to bind over the directory holding it: a read-only bind of the working directory, or of a write root, takes the whole tree the sandbox exists to let the command write. Such a layout is one the caller had before this library saw it, so the command is not refused for it either — as long as the chain reaches something, which is what leaves both its ends denied; a chain that reaches nothing through such a hop is refused instead (see above), unless it is a pointer's value, where the refusal would be the lockout it exists to avoid and the warning is the whole answer. Instead **one `SRT_DEBUG` warning per wrap** names the entry (or the pointer file, and what it names) and the hop and says it can be retargeted from inside the sandbox; both ends of the chain stay denied. A `denyWrite` entry naming the directory holding the link closes it, at the cost of that directory; moving the link inside the git directory, or spelling the pointer's path without it, closes it outright. Restoring the link after the command was considered and not done: a hop the command replaced with a directory cannot be put back by a symlink call, so a restore means removing what a command wrote inside the caller's own work tree; `cleanupAfterCommand` is the embedder's to call; and nothing of it closes the window while the command runs.
- The `.git/modules` walk has no depth bound: a submodule's name is its path and submodules nest, so how deep a tree goes is a question about the repository, and a submodule a hundred levels down has its hooks and config denied like any other. Each directory is visited once, keyed by where it really is, so a symlinked entry pointing back into the tree ends that branch rather than looping. What bounds the walk is time — as long as the `ripgrep` scan gets, since a tree that takes longer than that to walk is one the command about to run could have made — and running out of it refuses the command (`deny_scan_failed` on Linux, a `SubmoduleWalkBudgetError` on macOS) rather than sandboxing on the submodules the walk did reach. The same budget bounds the listing of `.git/worktrees`, the walk of a symlink chain a git directory entry leads into, and the walk of the path a `.git` pointer or a `commondir` names: a link target runs to 4095 bytes and a pointer file to 1 MiB, and every component of either is an `lstat`. The `.git` pointer files one scan lists share a single such budget between them. The budgets add up rather than overlap: at worst a wrap spends one on the working directory's own repository before the scan, the scan's own timeout, and then one each on the walk after a scan that could not read a directory, the git-directory walk `allowGitConfig` adds, all the pointer files together, and every nested repository found. The clock is looked at as the entries go by, so one directory a command filled with entries is stopped in the middle rather than at the end of it, and a walk made after the scan takes a budget of its own of the same length rather than what a slow scan left.
- A directory that looks like a git directory does not end the `.git/modules` walk. What makes it look like one is the name of a single entry, and a segment of a submodule's name (`vendor` of `vendor/lib`) is a directory a command can write, so a `HEAD` left in `vendor` would otherwise take the denies off `vendor/lib` from the next command on. The directory is denied as a git directory and what else it holds is walked as well. Only what git itself keeps there (`objects`, `refs`, `logs`, `hooks`, `info`, `worktrees`, `lfs`, `rr-cache`) is left at one listing each, and one of those is walked after all where that listing shows a `config` or a `hooks`, which a submodule named `vendor/lfs` has and git's own directory of that name does not.
- **Past what bubblewrap takes arguments for, the submodule denies are degraded (Linux).** A profile takes three of bubblewrap's 9000 words per mount. A repository with hundreds of submodules asks for more mounts than that — four denies and an ancestor pin for each submodule git directory, and for a checked-out one its `.git` pointer file and the pin of the directory holding it as well — and the ceiling is not this library's to lift. Refusing every command in such a repository is the wrong answer to it, so the wrap builds the profile, counts what bubblewrap will actually parse, and while that is past the cap degrades the submodule denies a step at a time and builds again, fail-closed and only as far as the count says it has to be. The working directory's own repository is served first and the nested repositories the scan found after it, in sorted order, so what is degraded is taken from the end:
  ...
  Linked worktrees' git directories are degraded by the same two steps, before the same repository's submodules: a worktree's git directory bound whole costs cross-worktree housekeeping (`git gc` expiring that worktree's reflog), and `.git/worktrees` bound whole costs `git worktree add`. A step that gives nothing back, because the git directories it took lie outside every write root and the read-only root holds them without a mount, ends the stepping: the next build is the last level, so the number of builds does not grow with how many submodules a repository holds.
...
- On macOS only the working directory's own `.git` file is followed, since nested pointers are matched by pattern; the working directory's own submodule git directories are enumerated exactly, while a nested repository's are matched as `.git/modules/<name>/`, which covers a single-segment submodule name. Nothing is ever collapsed there, so every submodule keeps its precise denies however many there are. A Seatbelt profile has no cap on entries of its own, but it travels on `sandbox-exec`'s command line and is bounded by `ARG_MAX` (1 MiB): enough entries under a `.git/modules`, which a command can write, and every later command fails at spawn with `E2BIG`, from the caller's own `spawn` and with no code from this library.
...
- Where the git directory takes no mount point at all — one this process cannot write, such as a root-owned vendored checkout or a read-only mount — the git directory is bound read-only whole instead. That needs nothing written to the host, costs what was already unwritable, and is also what stops the command from making the directory writable and creating the file itself. That bind is then the only mount the wrap puts inside the directory: a placeholder for another absent entry of it (a `config` or a `hooks` that is not there) would need a mount point made where none can be, or made by bubblewrap where this process could never remove it, so those entries are left to the bind, which denies them with everything else in the directory.
- A name found taken is read, and an empty one repaired, through the file that was looked at and never through its name: the git directory can be one a command already running writes, and a check of the path followed by a write to it would write wherever that command had pointed the name in between. Where the name has been given to something else meanwhile (a link, another file, a FIFO) that is left exactly as found and the git directory is bound read-only whole for the command instead; the cleanup likewise leaves a mount point whose name no longer leads to the file it checked.
...
- `deny_unresolvable_git_entry` — an entry of the working directory's OWN repository (`hooks`, `config`, `commondir` or `config.worktree` of the git directory its `.git` is or names) is a symlink whose chain reaches nothing (a loop, or a name no file can occupy), and one of the links it goes through sits in the working directory or a path this command may write, where no read-only bind can hold it. There is no landing to deny instead, so the command is refused rather than run with the link free; the message names the entry, the link and the directory holding it. Repairing the chain, moving the links inside the git directory, or a `denyWrite` entry naming the directory holding them, each lifts it. A pointer's value that reaches nothing is NOT this: see the bullet above. Neither is the same chain behind anything a command may have made (a nested repository the scan found, a submodule's git directory, a `modules` that was not there): those get the warning an unheld hop gets and the command runs, because refusing there would let a few ordinary writes lock every later command out, and would protect nothing the command could not already reach by creating a nested repository outright. The four entries of the working directory's own repository are denied at their own paths from the first wrap that saw it, so a link found at one is the caller's layout; what is left to plant is a repository created and linked this way by one command in a directory that had none.
...
**No whole-directory deny is ever emitted for the work tree.** Every one of them is a stand-in read off paths a sandboxed command can write - a `.git` pointer naming the checkout (`gitdir: .`, with one entry of the checkout a symlink, makes the checkout a git directory denied whole), a `.git/modules` entry linked at `../..`, a git directory entry pointing at the checkout, a directory left unreadable - and one that lands on the working directory, an ancestor of it, a write root or a directory holding one leaves every later command unable to write the project, with nothing inside the sandbox able to undo it. So: a directory that can be LISTED is dropped with a warning, since what the deny was holding keeps the deny paths of its own two ends; a directory that cannot be listed stands in for a tree nothing else covers, so the command is REFUSED instead (`deny_scan_failed`, naming the directory), which one `chmod` outside the sandbox lifts. Every deny path is judged by where it LANDS, since a bind resolves its destination: an entry that is itself a link to the checkout (`nested/.git/hooks -> ../..`) is dropped the same way, and the git directory around it, denied whole, is what holds the link. What a degrade adds is judged again, and a bind taken back there leaves the precise denies it stood for. The same call is made on macOS, where a literal entry is a subpath deny.

**A held chain hop is the same family, and the layout need not pre-exist:** a command can create a nested repository whose hooks entry leads through a link in a directory of its choosing (`mkdir -p x/.git; touch x/.git/HEAD; ln -s ../../src/hop x/.git/hooks; ln -s /nowhere src/hop`), and from the next wrap on `src/` is bound read-only for holding the link, which cannot be removed from inside it either. That is a denial of service by code that could as easily have deleted `src/`, and it is why the hop is held rather than the reason not to hold it — but it is not only the cost of a layout the caller already had. An `SRT_DEBUG` line names, for each directory bound this way, the link it is bound for and the entry that is reached through it.
...
- `git worktree remove`, `git worktree move`, `git worktree repair`, `git submodule deinit`, for the same reason, and `git worktree prune` (and the prune `git gc` runs) for a STALE worktree entry the repository still keeps, whose `.git/worktrees/<id>/` files are denied: it prints one `failed to delete` line, exits 0 all the same, removes what it can of that entry and leaves the rest for the next prune outside the sandbox; a detached `git gc --auto` leaves no `gc.log` behind over it (measured with git 2.55);
...
- on macOS, `git init` and `git clone` into a subdirectory, which create `.git/hooks/`. On Linux both succeed, since nothing is mounted where no `.git` was when the command was wrapped, and the repository they make is not covered until the next wrap;
...
**Known limit: a `core.hooksPath` or `include.path` into the work tree.** The denied `config` is never read. Where the repository's own config already sets `core.hooksPath` to a directory in the work tree (husky's `.husky/_`, generated and gitignored), git never looks in `.git/hooks`, and what it does run is ordinary writable content; the same goes for a file an `include.path` names. A sandboxed command cannot create that state, since `config` is denied, so it matters where the caller already has it.

**Known limit: a bare-repository layout in a subdirectory.** `HEAD`, `objects/`, `refs/` and `config` directly in a directory make it a repository git finds before the parent's `.git` when it is run from there (`safe.bareRepository` defaults to `all`). Nothing looks for that layout, so none of these denies apply to it.

**Known limit: sequencer state.** `<gitdir>/rebase-merge/git-rebase-todo` takes `exec` lines that `git rebase --continue` runs, and it stays writable. It needs that specific command run on the host afterwards.

**Known limit: a link that IS the git directory.** The denies above hold a link INSIDE a git directory or a `.git/modules`, `modules` itself included. A `.git` that is itself a symlink to a git directory elsewhere is not one of them: what it points at is denied as any git directory is, and the link itself sits in the working tree, which the sandbox is there to let the command write. Replacing it makes the repository a different one, which the next command's scan covers — but a host git run between the two commands follows the new link. There is nothing to bind over: a mount at the link's own path resolves it, and the only handle above it is the working tree root.
...
- any code `EACCES`: a directory the scan could not read holds an unknown tree, so it is denied whole and the rest of the listing still counts. WHICH directories is settled by a bounded walk of the working directory, to the scan's own depth and with a time budget of its own of the scan's length, not by reading the paths off stderr. A directory that is readable but not SEARCHABLE (mode 0400) lists its entries and lets nothing through them, so it is that directory, not the child whose own listing failed, that is denied: naming the child would ask bubblewrap for a file mount at what is really a directory and fail every later wrap in the tree.
...
A directory that sits at the very depth the scan stops at is a case of its own: ripgrep's walker opens one to list it and reports it when it cannot, while this walk stops a level above, so such an EACCES is accounted for with nothing to deny - nothing below it is in the scan's reach on either side. Without that, a project holding a directory another user owns exactly that far down (a `docker/data/postgres`, mode 0700) had every command refused.

Those three read a run that exited on its own. A run that was KILLED is refused whatever its stderr held — the timeout, an OOM kill, a signal to the process group — because its listing stops wherever the kill arrived and nothing on stderr says what lay past that; the message names the signal, and says whether the run had used the time it was given.
