...
- `allowNestedUserNamespaces` - Linux: let the sandboxed command create user namespaces of its own (boolean, default: false). By default it cannot, because the write denies do not survive a command that can: see "The command stays in the namespaces made for it" under Platform-Specific Dependencies. Turn it on only for a command that has to sandbox itself that way (a browser with its namespace sandbox, rootless podman or buildah, a nested bubblewrap, `unshare -r` in a build script), and treat `denyWrite` and the mandatory denies as not holding against that command.
